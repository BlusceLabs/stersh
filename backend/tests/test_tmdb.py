...
class TmdbGetTest(unittest.TestCase):
    """Test _tmdb_get by patching _get_client at the module level."""

    def _make_mock_client(self, status_code=200, json_body=None, raise_error=None):
        """Build a mock client whose .get() behaves as specified."""
        client = MagicMock()
        if raise_error:
    ...
    @patch.object(tmdb_mod, "tmdb_cache_get", return_value={"cached": True})
    def test_returns_cached_value(self, mock_get):
    ...
    def test_caches_miss_then_hit(self, mock_set, mock_get):
        ...
        mock_client = self._make_mock_client(json_body={"id": 123})
        with patch.object(tmdb_mod, "_get_client", return_value=mock_client):
            result = _run(tmdb_mod._tmdb_get("movie/123"))
    ...
    def test_raises_on_upstream_404(self, mock_get):
        ...
        mock_client = self._make_mock_client(
            status_code=404, json_body={"status_message": "Not found"}
        )
        with patch.object(tmdb_mod, "_get_client", return_value=mock_client):
    ...
    def test_raises_502_on_connection_error(self, mock_get):
        ...
        mock_client = self._make_mock_client(
            raise_error=httpx.RequestError("conn refused")
        )
        with patch.object(tmdb_mod, "_get_client", return_value=mock_client):
            with self.assertRaises(HTTPException) as ctx:
