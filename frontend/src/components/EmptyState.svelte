...
<div
  class="surface-elevated rounded-2xl p-8 sm:p-10 text-center {tone.ring}"
  role={variant === 'error' ? 'alert' : 'status'}
  aria-live={variant === 'error' ? 'assertive' : 'polite'}
>
  <div
    class="w-14 h-14 mx-auto mb-5 rounded-2xl {tone.bg} border {tone.ring} flex items-center justify-center {tone.text}"
    aria-hidden="true"
  >
    {#if icon === 'film'}
