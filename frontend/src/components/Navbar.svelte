...
<nav
  class="fixed bottom-0 left-0 right-0 z-50 px-3 pb-3 sm:pb-4 pointer-events-none"
  aria-label="Primary navigation"
>
  <div
    class="mx-auto rounded-2xl flex items-center justify-around h-14 sm:h-16 px-2 sm:px-3
      bg-[#09090b]/80 backdrop-blur-2xl border border-zinc-800/40 shadow-[0_8px_32px_rgba(0,0,0,0.8)]
      pointer-events-auto transition-all duration-300 max-w-lg sm:max-w-xl lg:max-w-2xl"
  >
    {#each navItems as item}
      ...
      >
        ...
        <svg 
          ...
          stroke="currentColor" 
          class="w-5 h-5 sm:w-5 sm:h-5 transition-all duration-200"
        >
          <path stroke-linecap="round" stroke-linejoin="round" d={item.icon} />
        </svg>
        
        <span class="text-[9px] sm:text-[10px] font-semibold tracking-wider leading-none whitespace-nowrap mt-0.5 opacity-80">
          {item.label}
        </span>
      </a>
    {/each}
