<script lang="ts">
  ...
  interface MediaDetails {
    ...
    credits?: { cast: CastMember[] }; videos?: { results: any[] };
  }
...
{:else}
  <div class="relative min-h-screen bg-surface-0 text-ink">

    <!-- ===== FULL-BLEED BACKDROP ===== -->
    {#if backdropUrl}
      <div class="absolute top-0 left-0 right-0 h-[70vh] md:h-[80vh] pointer-events-none overflow-hidden">
        <img src={backdropUrl} alt="" class="w-full h-full object-cover brightness-[0.15] scale-105 motion-safe:animate-[kenburns_25s_ease-out_infinite_alternate]" />
        <div class="absolute inset-0 bg-gradient-to-t from-surface-0 via-surface-0/60 to-transparent"></div>
        <div class="absolute inset-0 bg-gradient-to-b from-surface-0/30 via-transparent to-surface-0"></div>
        <div class="absolute inset-0 bg-gradient-to-r from-surface-0/80 via-transparent to-surface-0/40 md:to-transparent"></div>
        <div class="absolute inset-0 bg-radial-vignette"></div>
        <div class="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-surface-0 to-transparent"></div>
      </div>
    {/if}

    <!-- ===== HERO SECTION (edge-to-edge) ===== -->
    <section class="relative z-10 pt-[25vh] md:pt-[30vh] lg:pt-[35vh]">
      <div class="px-4 sm:px-6 lg:px-12 xl:px-20">
        <div class="flex flex-col md:flex-row gap-6 md:gap-10 items-end md:items-start pb-8">

          <!-- Poster -->
          <div class="hidden md:block w-[220px] lg:w-[280px] flex-shrink-0 relative group/poster">
          ...
          <!-- Info -->
          <div class="flex-1 min-w-0 md:pb-2">
            ...
            <!-- Title -->
            <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display font-black text-ink tracking-tighter leading-[1.05] text-cinematic-lg mb-2">
              {title}
            </h1>
    ...
    <!-- ===== FULL-WIDTH INFO BAR ===== -->
    <section class="relative z-10 border-y border-white/[0.04] bg-surface-0/80 backdrop-blur-xl -mt-2 md:mt-0">
      <div class="px-4 sm:px-6 lg:px-12 xl:px-20">
        <div class="flex flex-col md:flex-row gap-6 md:gap-10 py-8">

          <!-- Overview -->
          <div class="flex-1 min-w-0">
            {#if data?.overview}
              <h3 class="text-[11px] font-bold uppercase tracking-[0.2em] text-ink-subtle mb-3">Overview</h3>
            ...
            {#if cast.length > 0}
              <div class="mt-6">
                <h3 class="text-[11px] font-bold uppercase tracking-[0.2em] text-ink-subtle mb-3">Cast</h3>
                <div class="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scroll-x-clean select-none">
                  {#each cast as person}
                    <a href={`/search?q=${encodeURIComponent(person.name)}`} class="flex-shrink-0 w-[64px] text-center group/cast no-underline">
                      <div class="w-14 h-14 mx-auto rounded-full overflow-hidden bg-surface-1 ring-1 ring-white/[0.04] group-hover/cast:ring-brand-red/40 group-hover/cast:scale-105 transition-all duration-300 shadow-2">
                        ...
                        {:else}
                          <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-surface-1 to-surface-2 text-ink-subtle text-xs font-bold">{person.name.charAt(0)}</div>
                        {/if}
                      </div>
                      <p class="text-[10px] text-ink font-semibold leading-tight truncate w-full mt-1 group-hover/cast:text-white transition-colors">{person.name}</p>
                      <p class="text-[9px] text-ink-faint truncate w-full">{person.character || ''}</p>
                    </a>
                  {/each}
                </div>
              </div>
          ...
          <!-- Poster (mobile only) -->
          <div class="md:hidden w-32 flex-shrink-0 self-start">
            <div class="aspect-[2/3] rounded-xl overflow-hidden shadow-4 border border-white/[0.06] bg-surface-1">
              {#if posterUrl}
                <img src={posterUrl} alt={title} class="w-full h-full object-cover" loading="eager" />
              {:else}
                <div class="w-full h-full flex items-center justify-center bg-surface-1 text-ink-faint text-xs">No Poster</div>
              {/if}
            </div>
            {#if trailerKey}
              <button onclick={() => showTrailer = true} class="mt-3 w-full inline-flex items-center justify-center gap-2 px-3 py-2 glass-strong text-ink-secondary text-xs font-semibold rounded-lg hover:bg-white/[0.06] transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" /></svg>
                Trailer
              </button>
            {/if}
          </div>
    ...
    {#if mediaType === 'tv' && seasons.length > 0}
      <section class="relative z-10">
        <div class="px-4 sm:px-6 lg:px-12 xl:px-20 py-10">
            ...
            <div class="space-y-2">
              {#each seasonEpisodes as ep}
                <a
                  ...
                  class="group/ep flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.03] hover:border-white/[0.08] transition-all duration-300"
                >
                  <div class="flex-shrink-0 w-28 sm:w-40 aspect-video rounded-lg overflow-hidden bg-surface-1 relative">
                    {#if ep.still_path}
                  ...
                  <div class="flex-1 min-w-0 py-0.5">
                    ...
                    {#if ep.overview}
                      <p class="text-[11px] sm:text-xs text-ink-muted line-clamp-2 mt-1 leading-relaxed hidden sm:block">{ep.overview}</p>
                    {/if}
                  </div>
                </a>
              {/each}
